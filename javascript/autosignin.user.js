...
function newDayMonitorCallBack() {
	...
	if(now.getHours() == NEW_DAY_BEGIN_HOUR && now.getMinutes() == NEW_DAY_BEGIN_MINUTE) {
		console.log("[newDayMonitorCallBack]Good morning! What a new day!");
		if(now.getDay() >= 1 && now.getDay() <= 5) { // From Monday to Friday
			signInTime = new Date();
			var minutes = parseInt(4 * Math.random()) + 9;
